...
var GitHubActivity = (function() {
  ...
  var methods = {
      ...
      if (p.commits) {
        ...
        p.commits.forEach(function(d, i) {
          if (d.message.length > 50) {
            d.message = d.message.substring(0, 50) + '...';
          }
          if (i < 2) {
    ...
    getHeaderHTML: function(data) {
      if (data.name) {
        data.userNameLink = methods.renderLink(data.html_url, data.name + " GitHub");
      } else {
